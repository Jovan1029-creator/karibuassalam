// src\data\translations\tr.js
// Turkish copy for every English source string that passes through tx().
// Keys are the exact English strings used in components and data files.

export const tr = {
  /* ---------- navigation, shell, generic ---------- */
  "Skip to main content": "Ana içeriğe geç",
  "Karibu Assalam logo": "Karibu Assalam logosu",
  "Quick contact": "Hızlı iletişim",
  "Karibu Assalam Zanzibar travel hero": "Karibu Assalam Zanzibar seyahat görseli",
  "Karibu Assalam Eco-Village live status": "Karibu Assalam Eco-Village anlık durum",
  "Partner logos": "Partner logoları",
  logo: "logo",
  Explore: "Keşfet",
  "Learn more": "Daha fazla",
  "View details": "İncele",
  "Follow Us": "Bizi takip edin",
  Phone: "Telefon",
  Email: "E-posta",
  Message: "Mesaj",
  "Loading...": "Yükleniyor...",
  "Not Found": "Bulunamadı",
  "Page not found": "Sayfa bulunamadı",
  "The page you requested is not available. You can browse retreats or contact the team for help.":
    "Aradığınız sayfa mevcut değil. Kamplarımıza göz atabilir veya ekibimizle iletişime geçebilirsiniz.",
  "Browse Retreats": "Kampları İncele",
  "Contact Booking Team": "Rezervasyon Ekibiyle İletişim",
  "Need Help?": "Yardım ister misiniz?",
  "Need help with booking or travel plans? Message us on WhatsApp.":
    "Rezervasyon veya seyahat planlarınız için yardım mı lazım? WhatsApp'tan yazın.",
  "I need help with booking and travel information.":
    "Rezervasyon ve seyahat bilgileri konusunda yardıma ihtiyacım var.",
  WhatsApp: "WhatsApp",
  "Message us on WhatsApp": "WhatsApp'tan yazın",
  "Send via WhatsApp": "WhatsApp ile gönder",
  "Send via Email": "E-posta ile gönder",
  "Open WhatsApp": "WhatsApp'ı aç",
  "Open Email": "E-postayı aç",
  "open Instagram": "Instagram'ı aç",
  From: "Başlangıç",
  days: "gün",
  "day experience": "günlük program",
  Price: "Fiyat",
  Duration: "Süre",
  Base: "Merkez",
  Highlights: "Öne çıkanlar",
  "Highlights include:": "Öne çıkanlar:",

  /* ---------- home ---------- */
  "Karibu Assalam | Immersive Halal Travel & Volunteer Retreats in Zanzibar":
    "Karibu Assalam | Zanzibar'da Helal Seyahat ve Gönüllülük Kampları",
  "Karibu Assalam offers immersive sensory travel, volunteering, and halal-friendly retreat experiences in Zanzibar from the Karibu Assalam Eco-Village base.":
    "Karibu Assalam, Karibu Assalam Eco-Village merkezli olarak Zanzibar'da duyulara hitap eden seyahat, gönüllülük ve helal dostu kamp deneyimleri sunar.",
  "Explore Beautiful": "Keşfedin",
  Zanzibar: "Zanzibar",
  "Zanzibar's only community-run eco-village": "Zanzibar'ın topluluk temelli tek eko-köyü",
  "Immersive halal travel, eco-village stays, and community-based retreats from Karibu Assalam Eco-Village.":
    "Karibu Assalam Eco-Village'dan yola çıkan helal seyahat, eko-köy konaklaması ve topluluk temelli kamplar.",
  "A halal-friendly eco-village in Zanzibar where your stay funds local schools, women's training and youth camps. Running since 2017.":
    "Zanzibar'da helal dostu bir eko-köy: konaklamanız yerel okulları, kadın eğitim programlarını ve gençlik kamplarını destekliyor. 2017'den beri buradayız.",
  "Scroll to explore": "Keşfetmek için kaydırın",

  "Plan Your Stay": "Planınızı yapın",
  "Plan your stay": "Planınızı yapın",
  "Choose your Zanzibar experience": "Zanzibar deneyiminizi seçin",
  "Start with a retreat, family program, school camp, or nature-focused stay from the Karibu Assalam Eco-Village base.":
    "Karibu Assalam Eco-Village merkezli bir kamp, aile programı, okul kampı veya doğa odaklı konaklama ile başlayın.",

  "Local time in Zanzibar": "Zanzibar'da yerel saat",
  "Quiet hours in Zanzibar": "Zanzibar'da sessiz saatler",
  "Morning at Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village'da sabah",
  "Afternoon at Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village'da öğleden sonra",
  "Sunset hours in Zanzibar": "Zanzibar'da gün batımı saatleri",
  "Evening at Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village'da akşam",
  "Booking requests are open online": "Rezervasyon talepleri çevrimiçi açık",
  "Start a request": "Talep oluştur",
  "Stay connected": "Bize ulaşın",
  "Questions before booking?": "Rezervasyon öncesi sorularınız mı var?",

  "Life at Assalam": "Assalam'da hayat",
  "A glimpse of camp life": "Kamp hayatından bir kesit",
  "See the spaces, workshops, and shared moments behind each stay. Open Instagram for the latest updates from the team.":
    "Her konaklamanın arkasındaki mekânları, atölyeleri ve paylaşılan anları görün. Ekibin güncel paylaşımları için Instagram'a göz atın.",
  "For current photos and announcements, follow the team on Instagram.":
    "Güncel fotoğraflar ve duyurular için ekibi Instagram'da takip edin.",
  "Learning together": "Birlikte öğrenmek",
  "Eco-village by the coast": "Sahildeki eko-köy",
  "Rooms prepared for guests": "Misafirler için hazırlanan odalar",
  "Community moments": "Topluluk anları",
  "Shared meals": "Ortak sofralar",
  "Hands-on workshops": "Uygulamalı atölyeler",
  "Children gathered around a workshop table during a Karibu Assalam activity":
    "Bir Karibu Assalam etkinliğinde atölye masasının etrafında toplanan çocuklar",
  "Beachfront swing and gathering area at Karibu Assalam Eco-Village":
    "Karibu Assalam Eco-Village'da sahil kenarındaki salıncak ve buluşma alanı",
  "Prepared guest room at Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village'da hazırlanmış misafir odası",
  "Visitors and local children together during a community activity":
    "Bir topluluk etkinliğinde misafirler ve yerel çocuklar bir arada",
  "Guests and the kitchen team sharing a meal together":
    "Misafirler ve mutfak ekibi aynı sofrada",
  "Participants working together during a hands-on workshop":
    "Uygulamalı bir atölyede birlikte çalışan katılımcılar",

  "Our Partners": "Partnerlerimiz",
  "Trusted organizations and institutions featured as partners of Karibu Assalam.":
    "Karibu Assalam'ın birlikte çalıştığı güvenilir kurum ve kuruluşlar.",

  "Travel To Impact": "Etki için seyahat",
  "The Impact You Create": "Yarattığınız etki",
  "At Karibu Assalam, your journey becomes part of something bigger. Every stay, camp, and volunteer experience directly supports Assalam Community Foundation and helps create opportunities for local youth and families in Zanzibar.":
    "Karibu Assalam'da yolculuğunuz daha büyük bir şeyin parçası olur. Her konaklama, kamp ve gönüllülük deneyimi doğrudan Assalam Community Foundation'ı destekler ve Zanzibar'daki gençler ile aileler için fırsatlar yaratır.",
  "Your participation helps support": "Katılımınızın desteklediği alanlar",
  "Together, we are building a stronger, more connected community, where travel creates real impact.":
    "Birlikte, seyahatin gerçek bir etki yarattığı; daha güçlü ve daha bağlı bir topluluk kuruyoruz.",
  "Education programs at Assalam International School":
    "Assalam Uluslararası Okulu'ndaki eğitim programları",
  "Youth leadership and girls empowerment camps":
    "Gençlik liderliği ve kız çocukları güçlendirme kampları",
  "Empowerment projects for local men and women":
    "Yerel kadın ve erkekler için güçlendirme projeleri",
  "Orphan camps for children across Zanzibar":
    "Zanzibar genelindeki çocuklar için yetim kampları",
  "Community development initiatives": "Toplum kalkınma girişimleri",
  "Environmental and permaculture projects": "Çevre ve permakültür projeleri",
  "Cultural exchange and global citizenship programs":
    "Kültürel değişim ve küresel vatandaşlık programları",

  "Guest Voices": "Misafir yorumları",
  "Stories from people who joined Assalam": "Assalam'a katılanların hikâyeleri",
  "Travelers and volunteers describe Karibu Assalam as warm, meaningful, safe, and deeply connected to the community.":
    "Gezginler ve gönüllüler Karibu Assalam'ı sıcak, anlamlı, güvenli ve toplulukla iç içe olarak tanımlıyor.",

  "Experience, Volunteer, Balance": "Deneyim, Gönüllülük, Denge",
  "Travel days combine guided Zanzibar experiences with community involvement and active time.":
    "Program günleri rehberli Zanzibar deneyimlerini topluluk katılımı ve aktif zamanla birleştirir.",
  Experience: "Deneyim",
  Volunteer: "Gönüllülük",
  Balance: "Denge",
  "Sunsets and culture": "Gün batımları ve kültür",
  "Workshops and handcraft": "Atölyeler ve el sanatları",
  Markets: "Pazarlar",
  "Swim with dolphins": "Yunuslarla yüzme",
  "Fishermen routines": "Balıkçıların günlük hayatı",
  "Women vocational training schools": "Kadın meslek edindirme okulları",
  "Permaculture gardens": "Permakültür bahçeleri",
  Madrasas: "Medreseler",
  "Mobile libraries": "Gezici kütüphaneler",
  "Children's universities": "Çocuk üniversiteleri",
  "Dance and sports": "Dans ve spor",
  "Hiking and trekking in villages": "Köylerde yürüyüş ve doğa turları",
  "Ocean exploration at low tide": "Gelgit çekildiğinde okyanus keşfi",
  "Community time and guided activities": "Topluluk zamanı ve rehberli etkinlikler",

  "Welcome to Our Eco-Village": "Eko-köyümüze hoş geldiniz",
  "Explore the living spaces, dining experience, and campus environment that support each camp and retreat at Karibu Assalam Eco-Village.":
    "Karibu Assalam Eco-Village'daki her kampı destekleyen yaşam alanlarını, yemek deneyimini ve kampüs ortamını keşfedin.",
  Rooms: "Odalar",
  Food: "Yemek",
  Campus: "Kampüs",
  "Comfortable eco-village accommodation with ocean or garden views and renewable energy support.":
    "Okyanus veya bahçe manzaralı, yenilenebilir enerjiyle desteklenen konforlu eko-köy konaklaması.",
  "Farm-to-table meals prepared by a multicultural kitchen team, including shared dinners by the beach.":
    "Çok kültürlü mutfak ekibinin hazırladığı çiftlikten sofraya yemekler ve sahilde ortak akşam yemekleri.",
  "An oceanside campus with learning spaces, a communal area, and sustainability features.":
    "Öğrenme alanları, ortak yaşam alanı ve sürdürülebilirlik özellikleriyle deniz kenarında bir kampüs.",
  "Eco-village room exterior and lodging area": "Eko-köy oda dış görünümü ve konaklama alanı",
  "Fresh meal presentation from Karibu Assalam kitchen":
    "Karibu Assalam mutfağından taze bir yemek sunumu",
  "Assalam eco-village campus common space": "Assalam eko-köy kampüsünün ortak alanı",

  "A platform for volunteers and halal tourists.":
    "Gönüllüler ve helal turizm misafirleri için bir platform.",
  "Our travel approach is built around immersive sensory experiences, community connection, and balanced daily activity.":
    "Seyahat yaklaşımımız; duyulara hitap eden deneyimler, topluluk bağı ve dengeli bir günlük akış üzerine kurulu.",
  Sight: "Görmek",
  Touch: "Dokunmak",
  Hear: "Duymak",
  Smell: "Koklamak",
  "Sunsets, culture, markets, beaches, and Zanzibar's underwater beauty shape the visual rhythm of each trip.":
    "Gün batımları, kültür, pazarlar, plajlar ve Zanzibar'ın su altı güzelliği her yolculuğun görsel ritmini oluşturur.",
  "Hands-on workshops, handcraft activities, and community volunteering bring travelers closer to local life.":
    "Uygulamalı atölyeler, el sanatları ve topluluk gönüllülüğü misafirleri yerel hayata yaklaştırır.",
  "Listen to market energy, fishermen routines, and shared community moments throughout the day.":
    "Gün boyunca pazarın enerjisini, balıkçıların rutinini ve ortak topluluk anlarını dinleyin.",
  "Spice gardens, ocean air, and local meals create a sensory memory that lasts beyond the trip.":
    "Baharat bahçeleri, okyanus havası ve yerel yemekler yolculuktan sonra da kalan bir duyusal hafıza bırakır.",

  /* ---------- about ---------- */
  "About Karibu Assalam | Purpose-Driven Eco-Village in Zanzibar":
    "Karibu Assalam Hakkında | Zanzibar'da Amaç Odaklı Eko-Köy",
  "Karibu Assalam is a purpose-driven eco-village in Zanzibar created by Assalam Community Foundation, hosting travelers, volunteers, students, and youth groups.":
    "Karibu Assalam, Assalam Community Foundation tarafından kurulan; gezginleri, gönüllüleri, öğrencileri ve gençlik gruplarını ağırlayan amaç odaklı bir Zanzibar eko-köyüdür.",
  "What is Karibu Assalam?": "Karibu Assalam nedir?",
  "A purpose-driven eco-village in Zanzibar": "Zanzibar'da amaç odaklı bir eko-köy",
  "We host travelers, volunteers, students, and youth groups who want to experience Zanzibar through community engagement, nature, and cultural exchange.":
    "Zanzibar'ı topluluk katılımı, doğa ve kültürel değişim üzerinden deneyimlemek isteyen gezginleri, gönüllüleri, öğrencileri ve gençlik gruplarını ağırlıyoruz.",
  "Spice garden in Zanzibar": "Zanzibar'da baharat bahçesi",
  "Created by Assalam Community Foundation": "Assalam Community Foundation tarafından kuruldu",
  "Karibu Assalam is a purpose-driven eco-village in Zanzibar created by the NGO Assalam Community Foundation. By joining Karibu Assalam, you directly support local youth programs, education initiatives, and community development.":
    "Karibu Assalam, sivil toplum kuruluşu Assalam Community Foundation tarafından kurulan amaç odaklı bir Zanzibar eko-köyüdür. Karibu Assalam'a katılarak yerel gençlik programlarını, eğitim girişimlerini ve toplum kalkınmasını doğrudan desteklersiniz.",
  "Travelers participating in a purpose-driven activity in Zanzibar":
    "Zanzibar'da amaç odaklı bir etkinliğe katılan gezginler",
  "The experience is built for meaningful travel: community engagement, nature, cultural exchange, and responsible hospitality.":
    "Deneyim anlamlı seyahat için tasarlandı: topluluk katılımı, doğa, kültürel değişim ve sorumlu misafirperverlik.",
  "Why Karibu Assalam": "Neden Karibu Assalam",
  "Karibu Assalam camp participants": "Karibu Assalam kamp katılımcıları",
  "Camps since 2017": "2017'den beri kamplar",
  "Authentic local experience": "Otantik yerel deneyim",
  "Small and meaningful group experiences": "Küçük ve anlamlı grup deneyimleri",
  "Strong local community network": "Güçlü yerel topluluk ağı",
  "Travel that creates real impact": "Gerçek etki yaratan seyahat",
  "Nature and sustainability": "Doğa ve sürdürülebilirlik",
  "Cultural immersion": "Kültürel derinlik",
  "Halal-friendly environment": "Helal dostu ortam",
  "Safe and supportive atmosphere": "Güvenli ve destekleyici bir atmosfer",
  Impact: "Etki",
  "Every stay, camp, and volunteer experience directly supports the work of Assalam Community Foundation and helps create opportunities for local youth and families in Zanzibar.":
    "Her konaklama, kamp ve gönüllülük deneyimi Assalam Community Foundation'ın çalışmalarını doğrudan destekler ve Zanzibar'daki gençler ile aileler için fırsatlar yaratır.",
  "Community-focused program activities in Zanzibar":
    "Zanzibar'da topluluk odaklı program etkinlikleri",
  "Traveling with Purpose": "Amaçla seyahat etmek",
  "Hospitality connected to community benefit": "Toplum yararına bağlı misafirperverlik",
  "Karibu Assalam combines guided Zanzibar experiences with volunteer opportunities, balanced daily activities, and sustainable tourism values. The goal is not only to visit, but to contribute responsibly while learning through community connection.":
    "Karibu Assalam; rehberli Zanzibar deneyimlerini gönüllülük fırsatları, dengeli günlük etkinlikler ve sürdürülebilir turizm değerleriyle birleştirir. Amaç yalnızca ziyaret etmek değil, toplulukla bağ kurarak öğrenirken sorumlu biçimde katkı sunmaktır.",
  "Spice garden pathway in Zanzibar": "Zanzibar'da baharat bahçesi yolu",

  /* ---------- retreats ---------- */
  "Retreats & Camps at Karibu Assalam Eco-Village | Karibu Assalam":
    "Karibu Assalam Eco-Village Kampları | Karibu Assalam",
  "Explore Karibu Assalam retreats and camps at Karibu Assalam Eco-Village with guided tours, meals, accommodations, airport transport, and seminars/activities.":
    "Karibu Assalam Eco-Village'daki Karibu Assalam kamplarını keşfedin: rehberli turlar, yemekler, konaklama, havalimanı transferi ve seminer/etkinlikler.",
  Retreats: "Kamplar",
  "Retreats & Camps at Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village'da kamplar",
  "Karibu Assalam offers retreats and camps from Karibu Assalam Eco-Village, with the provided framing of the only eco-village in Zanzibar.":
    "Karibu Assalam, Zanzibar'ın tek eko-köyü olan Karibu Assalam Eco-Village merkezli kamplar düzenler.",
  "Nature retreat scenery at Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village'da doğa kampı manzarası",
  "Our retreats and camps": "Kamplarımız",
  "Package details are confirmed with the team": "Paket detayları ekiple birlikte netleşir",
  "Each retreat, camp, and stay can have different inclusions and exclusions. Contact the Karibu Assalam team for the current open retreat details before booking.":
    "Her kamp ve konaklamanın dahil olan ve olmayan hizmetleri farklılık gösterebilir. Rezervasyon öncesinde güncel detaylar için Karibu Assalam ekibiyle iletişime geçin.",
  "Accommodations-only booking is also available": "Sadece konaklama rezervasyonu da mümkün",
  "Guests can book accommodations at Karibu Assalam Eco-Village without attending a camp.":
    "Misafirler kampa katılmadan da Karibu Assalam Eco-Village'da konaklama rezervasyonu yapabilir.",
  "Contact for accommodation booking": "Konaklama rezervasyonu için iletişim",
  "Retreat Detail": "Kamp detayı",
  "Retreat not found": "Kamp bulunamadı",
  "The requested retreat page is unavailable.": "İstenen kamp sayfası mevcut değil.",
  "Back to Retreats": "Kamplara dön",
  "retreat preview": "kamp önizlemesi",
  "Karibu Assalam Retreats": "Karibu Assalam Kampları",
  "at Karibu Assalam Eco-Village includes accommodations, meals, activities, airport transportation, and guided tours with Karibu Assalam.":
    "Karibu Assalam Eco-Village'da konaklama, yemekler, etkinlikler, havalimanı transferi ve Karibu Assalam rehberli turlarını içerir.",
  "Package details": "Paket detayları",
  "Inclusions and exclusions can vary by package and date. Please confirm current details with the Karibu Assalam team before booking.":
    "Dahil olan ve olmayan hizmetler pakete ve tarihe göre değişebilir. Rezervasyon öncesinde güncel detayları Karibu Assalam ekibiyle teyit edin.",
  "See sample day-by-day plan": "Örnek gün gün programa bakın",
  "7-Day Itinerary": "7 Günlük Program",
  "Sample Day 1-Day 7 Itinerary": "Örnek 1-7. Gün Programı",
  "Day-by-day structure shown below uses only provided retreat facts and general program wording for unspecified days.":
    "Aşağıdaki gün gün akış, paylaşılan kamp bilgilerine dayanır; belirtilmeyen günler için genel program ifadeleri kullanılmıştır.",
  "Booking & Trip Manual": "Rezervasyon ve seyahat kılavuzu",
  "Book this retreat": "Bu kampa katıl",
  "Contact Karibu Assalam for booking details, availability, and next steps for":
    "Rezervasyon detayları, uygunluk ve sonraki adımlar için Karibu Assalam ile iletişime geçin:",
  "Open booking form": "Rezervasyon formunu aç",
  "Trip manual": "Seyahat kılavuzu",
  "Open the trip manual PDF for this retreat.": "Bu kampın seyahat kılavuzunu (PDF) açın.",
  "Open Trip Manual (PDF)": "Seyahat Kılavuzunu Aç (PDF)",
  "No trip manual link is published for this retreat yet. Contact the team for current details.":
    "Bu kamp için henüz seyahat kılavuzu yayımlanmadı. Güncel bilgi için ekibimizle iletişime geçin.",
  "Related retreats": "Benzer kamplar",
  "View all retreats": "Tüm kampları gör",
  "I would like booking details for": "Şu kamp için rezervasyon bilgisi almak istiyorum:",

  "Kindness Camp": "İyilik Kampı",
  "Ramadan Camp": "Ramazan Kampı",
  "Family Tour": "Aile Turu",
  "Cultural Heritage Tour": "Kültürel Miras Turu",
  "School Camp": "Okul Kampı",
  "Nature Retreat": "Doğa Kampı",
  "A service-centered retreat combining community volunteering, seminars, and guided Zanzibar experiences.":
    "Topluluk gönüllülüğünü, seminerleri ve rehberli Zanzibar deneyimlerini birleştiren hizmet odaklı bir kamp.",
  "A Ramadan camp centered on community support, zakat activities, and shared iftar in Zanzibar.":
    "Zanzibar'da topluluk desteği, zekât faaliyetleri ve ortak iftar etrafında şekillenen bir Ramazan kampı.",
  "A family-oriented Zanzibar program with guided tours, shared activities, and eco-village accommodation.":
    "Rehberli turlar, ortak etkinlikler ve eko-köy konaklaması içeren aile odaklı bir Zanzibar programı.",
  "A heritage-focused retreat exploring cultural sites and historical context across Zanzibar.":
    "Zanzibar genelindeki kültürel mekânları ve tarihi arka planı keşfeden miras odaklı bir kamp.",
  "A structured school camp with hands-on learning, instructor guidance, and a clear daily flow.":
    "Uygulamalı öğrenme, eğitmen rehberliği ve net bir günlük akış sunan yapılandırılmış bir okul kampı.",
  "A 7-day nature retreat focused on permaculture, sustainability, and eco-village living.":
    "Permakültür, sürdürülebilirlik ve eko-köy yaşamına odaklanan 7 günlük bir doğa kampı.",

  Accommodations: "Konaklama",
  "Three daily meals": "Günde üç öğün yemek",
  "Participation in seminars/activities": "Seminer ve etkinliklere katılım",
  "Airport transportation": "Havalimanı transferi",
  "Guided tours": "Rehberli turlar",
  "Kizimkazi Village": "Kizimkazi Köyü",
  "Spice Garden": "Baharat Bahçesi",
  "Stone Town": "Stone Town",
  "Kanga Village": "Kanga Köyü",
  "Jozani Forest": "Jozani Ormanı",
  "Food packages": "Gıda paketleri",
  "Zakat distribution": "Zekât dağıtımı",
  "Iftar with local community in Kanga Village (~6:30 pm)":
    "Kanga Köyü'nde yerel toplulukla iftar (yaklaşık 18.30)",
  "Prison island tour": "Prison Island turu",
  "Dimbani Mosque": "Dimbani Camii",
  "Stone Town heritage elements": "Stone Town miras öğeleri",
  "Hands-on learning": "Uygulamalı öğrenme",
  Instructors: "Eğitmenler",
  "Typical Day flow": "Tipik gün akışı",
  Permaculture: "Permakültür",
  Sustainability: "Sürdürülebilirlik",

  "Arrival and airport transfer": "Varış ve havalimanı transferi",
  "Check in at Karibu Assalam Eco-Village and settle into the retreat schedule.":
    "Karibu Assalam Eco-Village'a giriş yapın ve kamp programına uyum sağlayın.",
  "Community activities and workshops": "Topluluk etkinlikleri ve atölyeler",
  "Structured activities, shared meals, and guided sessions.":
    "Planlı etkinlikler, ortak sofralar ve rehberli oturumlar.",
  "Guided activities from Karibu Assalam Eco-Village base":
    "Karibu Assalam Eco-Village merkezli rehberli etkinlikler",
  "Seminars and activities linked to the retreat theme.":
    "Kamp temasına bağlı seminerler ve etkinlikler.",
  "Seminars and reflection sessions": "Seminerler ve değerlendirme oturumları",
  "Program sessions and group reflection.": "Program oturumları ve grup değerlendirmesi.",
  "Themed activities and guided group time.": "Temalı etkinlikler ve rehberli grup zamanı.",
  "Guided tours and shared learning": "Rehberli turlar ve ortak öğrenme",
  "Guided tour time with learning-focused discussion.":
    "Öğrenme odaklı sohbetlerle rehberli tur zamanı.",
  "Departure and airport transfer": "Ayrılış ve havalimanı transferi",
  "Closing session and airport transportation.": "Kapanış oturumu ve havalimanı transferi.",
  "Arrival and orientation": "Varış ve oryantasyon",
  "Airport transfer, check-in, and retreat orientation at Karibu Assalam Eco-Village.":
    "Havalimanı transferi, giriş işlemleri ve Karibu Assalam Eco-Village'da kamp oryantasyonu.",
  "Guided camp activities and community-focused time in Kizimkazi Village.":
    "Kizimkazi Köyü'nde rehberli kamp etkinlikleri ve topluluk odaklı zaman.",
  "A guided Spice Garden visit as part of the 7-day plan.":
    "7 günlük programın parçası olarak rehberli Baharat Bahçesi ziyareti.",
  "Guided exploration and program activities in Stone Town.":
    "Stone Town'da rehberli keşif ve program etkinlikleri.",
  "Community activities and camp sessions connected to Kanga Village.":
    "Kanga Köyü ile bağlantılı topluluk etkinlikleri ve kamp oturumları.",
  "Guided excursion to Jozani Forest and shared reflection.":
    "Jozani Ormanı'na rehberli gezi ve ortak değerlendirme.",
  "Closing and departure": "Kapanış ve ayrılış",
  "Final sessions, wrap-up, and airport transportation.":
    "Son oturumlar, kapanış ve havalimanı transferi.",
  "Food package preparation": "Gıda paketi hazırlığı",
  "Prepare and organize food packages as part of camp activities.":
    "Kamp etkinlikleri kapsamında gıda paketlerini hazırlayın ve düzenleyin.",
  "Participate in zakat distribution activities with the camp team.":
    "Kamp ekibiyle birlikte zekât dağıtım faaliyetlerine katılın.",
  "Kanga Village iftar (~6:30 pm)": "Kanga Köyü'nde iftar (yaklaşık 18.30)",
  "Iftar with the local community in Kanga Village at approximately 6:30 pm.":
    "Kanga Köyü'nde yerel toplulukla yaklaşık 18.30'da iftar.",
  "Guided prison island tour as part of the family program.":
    "Aile programı kapsamında rehberli Prison Island turu.",
  "Family activities": "Aile etkinlikleri",
  "Shared family activities and guided exploration.":
    "Ortak aile etkinlikleri ve rehberli keşif.",
  "Guided sessions focused on Stone Town heritage elements.":
    "Stone Town miras öğelerine odaklanan rehberli oturumlar.",
  "Visit and learning activities connected to Dimbani Mosque.":
    "Dimbani Camii ile bağlantılı ziyaret ve öğrenme etkinlikleri.",
  "Instructor-led sessions follow a clear daily learning rhythm.":
    "Eğitmen eşliğindeki oturumlar net bir günlük öğrenme ritmi izler.",
  "Practical learning activities guided by instructors.":
    "Eğitmenler rehberliğinde uygulamalı öğrenme etkinlikleri.",
  "Instructor sessions": "Eğitmen oturumları",
  "Workshops and learning sessions with instructors.":
    "Eğitmenlerle atölyeler ve öğrenme oturumları.",
  "Arrival and eco-village introduction": "Varış ve eko-köy tanıtımı",
  "Airport transfer and orientation at Karibu Assalam Eco-Village.":
    "Havalimanı transferi ve Karibu Assalam Eco-Village'da oryantasyon.",
  "Permaculture introduction": "Permakültüre giriş",
  "Guided sessions introducing permaculture practices.":
    "Permakültür uygulamalarını tanıtan rehberli oturumlar.",
  "Sustainability workshops": "Sürdürülebilirlik atölyeleri",
  "Workshops focused on sustainability and community benefit.":
    "Sürdürülebilirlik ve toplum yararına odaklanan atölyeler.",
  "Nature-based guided activities": "Doğa temelli rehberli etkinlikler",
  "Guided activities linked to the natural environment.":
    "Doğal çevreyle bağlantılı rehberli etkinlikler.",
  "Permaculture practice": "Permakültür uygulaması",
  "Practical permaculture-focused sessions and seminars.":
    "Permakültür odaklı uygulamalı oturumlar ve seminerler.",
  "Community learning wrap-up": "Ortak öğrenme değerlendirmesi",
  "Shared learning and sustainability reflections.":
    "Ortak öğrenme ve sürdürülebilirlik üzerine değerlendirme.",

  /* ---------- zanzibar ---------- */
  "Zanzibar Experiences | Karibu Assalam": "Zanzibar Deneyimleri | Karibu Assalam",
  "Explore Zanzibar experiences including Spice Garden visits, East Coast tours, Blue Safari, Turtle Cave, and historic Stone Town highlights.":
    "Baharat Bahçesi ziyaretleri, Doğu Sahili turları, Mavi Safari, Kaplumbağa Mağarası ve tarihi Stone Town duraklarını içeren Zanzibar deneyimlerini keşfedin.",
  "Guided Zanzibar experiences": "Rehberli Zanzibar deneyimleri",
  "Discover curated Zanzibar modules that combine culture, coastline, and heritage landmarks.":
    "Kültürü, sahil şeridini ve miras noktalarını birleştiren özenle seçilmiş Zanzibar modüllerini keşfedin.",
  "Stone Town and Spice Garden experience in Zanzibar":
    "Zanzibar'da Stone Town ve Baharat Bahçesi deneyimi",
  "Experience Modules": "Deneyim modülleri",
  "A guided Spice Garden visit introduces Zanzibar's spice heritage through plants, scents, local farming knowledge, and cultural storytelling.":
    "Rehberli Baharat Bahçesi ziyareti; bitkiler, kokular, yerel tarım bilgisi ve kültürel anlatılar üzerinden Zanzibar'ın baharat mirasını tanıtır.",
  "Spice garden excursion scene": "Baharat bahçesi gezisinden bir kare",
  "East Coast Tour": "Doğu Sahili Turu",
  "Explore Zanzibar's east coast and include the well-known Rock Restaurant experience, where access depends on the tide.":
    "Zanzibar'ın doğu sahilini keşfedin; erişimi gelgite bağlı olan ünlü Rock Restaurant deneyimi de programa dahil edilebilir.",
  "East Coast Tour scenery in Zanzibar": "Zanzibar'da Doğu Sahili Turu manzarası",
  "Blue Safari": "Mavi Safari",
  "Blue Safari features snorkeling, marine life, and sandbank-focused ocean experiences.":
    "Mavi Safari; şnorkelle dalış, deniz canlıları ve kum adacığı odaklı okyanus deneyimleri sunar.",
  "Blue Safari ocean excursion in Zanzibar": "Zanzibar'da Mavi Safari okyanus gezisi",
  "Swimming with Turtles": "Kaplumbağalarla yüzme",
  "Turtle Cave offers swimming with turtles, feeding encounters, and relaxation-focused nature escapes.":
    "Kaplumbağa Mağarası; kaplumbağalarla yüzme, besleme deneyimi ve dinlendirici doğa molaları sunar.",
  "Turquoise ocean scenery for Turtle Cave excursion":
    "Kaplumbağa Mağarası gezisi için turkuaz okyanus manzarası",
  "Stonetown Historical Site": "Stone Town tarihi bölgesi",
  "Historical site view in Stone Town Zanzibar":
    "Zanzibar Stone Town'da tarihi bölge görünümü",
  "Anglican Cathedral": "Anglikan Katedrali",
  "House of Wonders": "Harikalar Evi",
  "Palace Museum": "Saray Müzesi",
  "Old Fort": "Eski Kale",
  "Darajani Market": "Darajani Pazarı",
  "Hamamni Baths": "Hamamni Hamamları",
  "Freddie Mercury’s house": "Freddie Mercury'nin evi",

  /* ---------- eco-village pages ---------- */
  "Eco-Village": "Eko-Köy",
  "Eco-Village Campus | Karibu Assalam": "Eko-Köy Kampüsü | Karibu Assalam",
  "Explore the Karibu Assalam eco-village campus with an international school setting, permaculture garden, renewable energy, and oceanside learning spaces.":
    "Uluslararası okul ortamı, permakültür bahçesi, yenilenebilir enerji ve deniz kenarındaki öğrenme alanlarıyla Karibu Assalam eko-köy kampüsünü keşfedin.",
  "Assalam eco-village campus": "Assalam eko-köy kampüsü",
  "An oceanside, multicultural campus environment designed for learning, community activities, and sustainability-focused living.":
    "Öğrenme, topluluk etkinlikleri ve sürdürülebilirlik odaklı yaşam için tasarlanmış, deniz kenarında çok kültürlü bir kampüs ortamı.",
  "Assalam eco-village campus gathering area": "Assalam eko-köy kampüsü buluşma alanı",
  "Campus Features": "Kampüs özellikleri",
  "Campus Life": "Kampüs hayatı",
  "International School": "Uluslararası okul",
  "Permaculture Garden": "Permakültür bahçesi",
  "Renewable Energy": "Yenilenebilir enerji",
  Oceanside: "Deniz kenarı",
  Multicultural: "Çok kültürlü",
  "The campus includes renewable energy systems such as solar panels, an amphitheater for group gatherings, a communal eating area, and a pool designed for the youngest learners.":
    "Kampüste güneş panelleri gibi yenilenebilir enerji sistemleri, grup buluşmaları için bir amfi, ortak yemek alanı ve en küçük öğrenciler için tasarlanmış bir havuz bulunur.",
  "Its oceanside setting supports community-based learning, shared meals, and daily activities in a multicultural environment.":
    "Deniz kenarındaki konumu; çok kültürlü bir ortamda topluluk temelli öğrenmeyi, ortak sofraları ve günlük etkinlikleri destekler.",

  "Rooms at Karibu Assalam Eco-Village | Karibu Assalam": "Karibu Assalam Eco-Village Odaları | Karibu Assalam",
  "Explore room features at Karibu Assalam Eco-Village including 25 m2 spaces, 5 bed capacity, AC, renewable energy, and ocean or garden views.":
    "Karibu Assalam Eco-Village oda özelliklerini keşfedin: 25 m², 5 yatak kapasitesi, klima, yenilenebilir enerji ve okyanus ya da bahçe manzarası.",
  "Eco-Village Rooms": "Eko-Köy Odaları",
  "Comfortable stays at Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village'da konforlu konaklama",
  "Rooms are designed to support camp and retreat stays with practical comfort and proximity to the sea.":
    "Odalar; pratik konfor ve denize yakınlıkla kamp konaklamalarını destekleyecek şekilde tasarlandı.",
  "Rooms area at Karibu Assalam Eco-Village in Zanzibar": "Zanzibar Karibu Assalam Eco-Village'da odalar bölümü",
  "Room Features": "Oda özellikleri",
  "Room Description": "Oda açıklaması",
  "25 m2": "25 m²",
  "5 bed capacity": "5 yatak kapasitesi",
  AC: "Klima",
  "Ocean or Garden View": "Okyanus veya bahçe manzarası",
  "Rooms include a balcony, mosquito nets, and a location close to the sea. Each room setup includes a bathroom and bed configurations suitable for group and retreat stays.":
    "Odalarda balkon, cibinlik ve denize yakın bir konum bulunur. Her oda düzeninde banyo ve grup konaklamalarına uygun yatak seçenekleri yer alır.",
  "Views may face the ocean or garden, and the accommodation environment is supported by renewable energy systems.":
    "Manzara okyanusa ya da bahçeye bakabilir; konaklama alanı yenilenebilir enerji sistemleriyle desteklenir.",
  "Assalam eco-village room and campus view": "Assalam eko-köy odası ve kampüs manzarası",

  "Food at Karibu Assalam Eco-Village | Karibu Assalam": "Karibu Assalam Eco-Village'da Yemek | Karibu Assalam",
  "Karibu Assalam food service includes farm-to-table meals, hygienic preparation, multicultural flavors, and beach dinners with sunset views.":
    "Karibu Assalam yemek hizmeti; çiftlikten sofraya yemekler, hijyenik hazırlık, çok kültürlü lezzetler ve gün batımı manzaralı sahil akşam yemekleri içerir.",
  "Eco-Village Food": "Eko-Köy Mutfağı",
  "Meals prepared for camp and retreat life": "Kamp hayatı için hazırlanan yemekler",
  "Daily dining at Karibu Assalam Eco-Village is built around fresh meals, hygienic preparation, and shared experiences.":
    "Karibu Assalam Eco-Village'daki günlük yemek düzeni taze yemekler, hijyenik hazırlık ve ortak deneyimler üzerine kuruludur.",
  "Farm-to-table meal prepared at Karibu Assalam Eco-Village":
    "Karibu Assalam Eco-Village'da hazırlanan çiftlikten sofraya bir yemek",
  "Kitchen & Dining Features": "Mutfak ve yemek özellikleri",
  "Dining Experience": "Yemek deneyimi",
  "Farm to Table": "Çiftlikten sofraya",
  Delicious: "Lezzetli",
  Hygienic: "Hijyenik",
  "Talented Chefs": "Yetenekli şefler",
  "Camp and retreat guests are served three meals daily, prepared by talented chefs in a hygienic kitchen environment with multicultural food influences.":
    "Kamp misafirlerine, hijyenik bir mutfak ortamında yetenekli şefler tarafından hazırlanan ve çok kültürlü lezzetlerden ilham alan günde üç öğün yemek sunulur.",
  "Dining also includes beach dinners with sunset views, creating a shared mealtime experience alongside the program schedule.":
    "Yemek düzeni, program akışının yanı sıra ortak bir sofra deneyimi yaratan gün batımı manzaralı sahil akşam yemeklerini de içerir.",

  /* ---------- faq ---------- */
  "FAQ | Karibu Assalam": "SSS | Karibu Assalam",
  FAQ: "SSS",
  "Frequently asked questions": "Sıkça sorulan sorular",
  "Frequently asked questions about reservations, deposits, check-in, airport transfers, meals, Wi-Fi, rooms, family stays, tours, and Karibu Assalam facilities.":
    "Rezervasyon, kapora, giriş işlemleri, havalimanı transferi, yemekler, Wi-Fi, odalar, aile konaklamaları, turlar ve Karibu Assalam olanakları hakkında sıkça sorulan sorular.",
  "Helpful guidance for planning your retreat, camp, or campus visit.":
    "Kampınızı veya kampüs ziyaretinizi planlarken işinize yarayacak bilgiler.",
  "Group activity associated with Karibu Assalam programs":
    "Karibu Assalam programlarından bir grup etkinliği",
  "Booking & Cancellation": "Rezervasyon ve iptal",
  "What is the cancellation policy?": "İptal koşulları nedir?",
  "Camp and retreat bookings require a 20% deposit. Individual stays can be paid on arrival.":
    "Kamp rezervasyonlarında %20 kapora alınır. Bireysel konaklamalar varışta ödenebilir.",
  "How can I make a reservation?": "Nasıl rezervasyon yapabilirim?",
  "You can book directly through our website or contact the team via WhatsApp.":
    "Doğrudan web sitemizden rezervasyon yapabilir veya WhatsApp üzerinden ekibimize ulaşabilirsiniz.",
  "What are the check-in and check-out times?": "Giriş ve çıkış saatleri nedir?",
  "Check-in and check-out times can be arranged according to your needs, subject to availability.":
    "Giriş ve çıkış saatleri, uygunluk durumuna bağlı olarak ihtiyacınıza göre ayarlanabilir.",
  "Travel Support": "Seyahat desteği",
  "Do you offer airport transfers?": "Havalimanı transferi sunuyor musunuz?",
  "Yes, transfers can be arranged upon request.": "Evet, talep üzerine transfer düzenlenebilir.",
  "What activities and excursions are available?": "Hangi etkinlik ve geziler mevcut?",
  "We offer Blue Safari, Turtle Cave visits, snorkeling, cultural tours, and other Zanzibar excursions.":
    "Mavi Safari, Kaplumbağa Mağarası ziyaretleri, şnorkelle dalış, kültür turları ve diğer Zanzibar gezilerini sunuyoruz.",
  "Do you offer private tours or custom excursions?": "Özel tur veya kişiye özel gezi düzenliyor musunuz?",
  "Yes, private and customized tours can be arranged.":
    "Evet, özel ve kişiye özel turlar düzenlenebilir.",
  "Stay, Meals & Facilities": "Konaklama, yemek ve olanaklar",
  "Is breakfast and dinner included in the stay?": "Kahvaltı ve akşam yemeği konaklamaya dahil mi?",
  "Yes, complimentary breakfast and dinner are included.":
    "Evet, kahvaltı ve akşam yemeği ücretsiz olarak dahildir.",
  "Is Wi-Fi available at the property?": "Tesiste Wi-Fi var mı?",
  "Yes, free Wi-Fi is available.": "Evet, ücretsiz Wi-Fi mevcuttur.",
  "Do you have air-conditioned rooms?": "Klimalı odalarınız var mı?",
  "Yes, all rooms are air-conditioned.": "Evet, tüm odalar klimalıdır.",
  "Is the property family-friendly?": "Tesis aileler için uygun mu?",
  "Yes, families are welcome.": "Evet, aileleri memnuniyetle ağırlıyoruz.",
  "Do you offer vegetarian or special meals?": "Vejetaryen veya özel menü seçeneğiniz var mı?",
  "Yes, vegetarian options are available on our menu. Gluten-free meals are not currently offered.":
    "Evet, menümüzde vejetaryen seçenekler bulunur. Glutensiz menü şu anda sunulmamaktadır.",
  "Is the property close to the beach?": "Tesis plaja yakın mı?",
  "Yes, the property is right on the oceanfront and also just a 10-minute walk from the sandy beach.":
    "Evet, tesis doğrudan okyanus kıyısındadır ve kum plaja yürüyerek yalnızca 10 dakika uzaklıktadır.",
  "Do you provide laundry service?": "Çamaşır hizmeti veriyor musunuz?",
  "Yes, laundry service is available upon request.":
    "Evet, talep üzerine çamaşır hizmeti sunulmaktadır.",
  Preparation: "Hazırlık",
  "What should I pack?": "Yanıma ne almalıyım?",
  "Pack travel essentials such as a reusable water bottle, sunscreen, mosquito repellent, and water shoes, along with your regular clothing and personal items. Single-use plastics are prohibited, so reusable items are important.":
    "Günlük kıyafet ve kişisel eşyalarınızın yanında yeniden kullanılabilir matara, güneş kremi, sivrisinek kovucu ve deniz ayakkabısı gibi temel seyahat malzemelerini getirin. Tek kullanımlık plastikler yasak olduğu için yeniden kullanılabilir ürünler önemlidir.",
  "Which languages are used during camps?": "Kamplarda hangi diller konuşuluyor?",
  "English, Turkish, and Swahili may be used, depending on the camp and team on site.":
    "Kampa ve sahadaki ekibe göre İngilizce, Türkçe ve Svahili dilleri kullanılabilir.",

  /* ---------- contact ---------- */
  "Contact & Booking | Karibu Assalam": "İletişim ve Rezervasyon | Karibu Assalam",
  "Contact Karibu Assalam to ask questions, request a campus visit, or send a booking request via WhatsApp or email.":
    "Soru sormak, kampüs ziyareti talep etmek veya WhatsApp ya da e-posta ile rezervasyon talebi göndermek için Karibu Assalam ile iletişime geçin.",
  Contact: "İletişim",
  "Contact and booking support": "İletişim ve rezervasyon desteği",
  "Tell us what you are planning and the team will reply, usually within one day.":
    "Ne planladığınızı yazın; ekibimiz genellikle bir gün içinde yanıt versin.",
  "Traveling with purpose scene in Zanzibar": "Zanzibar'da amaçla seyahat sahnesi",
  "Contact Karibu Assalam": "Karibu Assalam ile iletişim",
  "Share your travel plans and the team can guide you on booking, camp details, or campus visit requests.":
    "Seyahat planınızı paylaşın; ekibimiz rezervasyon, kamp detayları veya kampüs ziyareti konusunda size yol göstersin.",
  "Direct contact": "Doğrudan iletişim",
  "Open structured booking form": "Ayrıntılı rezervasyon formunu aç",
  "Booking intent detected for": "Şu kamp için rezervasyon talebi:",
  "I am interested in": "Şununla ilgileniyorum:",
  "Your Name": "Adınız",
  "Your Email": "E-posta adresiniz",
  "Phone Number": "Telefon numaranız",
  "Your Name is required.": "Adınızı yazın.",
  "Your Email is required.": "E-posta adresinizi yazın.",
  "Phone Number is required.": "Telefon numaranızı yazın.",
  "Message is required.": "Lütfen bir mesaj yazın.",
  "Please enter a valid email address.": "Geçerli bir e-posta adresi girin.",
  "Please fix the highlighted fields and try again.":
    "İşaretli alanları düzeltip tekrar deneyin.",
  "Tell us about your travel plans or question.":
    "Seyahat planınızı veya sorunuzu bize anlatın.",
  Website: "Web sitesi",
  "Request received. Please use the visible contact channels if you need immediate support.":
    "Talebiniz alındı. Acil destek gerekiyorsa lütfen görünen iletişim kanallarını kullanın.",
  "Send message": "Mesajı gönder",
  "Sending...": "Gönderiliyor...",
  "Thanks — we have your message. The team usually replies within one day.":
    "Teşekkürler, mesajınızı aldık. Ekibimiz genellikle bir gün içinde yanıt verir.",
  "We could not send your message just now. Please use WhatsApp or email below and we will get straight back to you.":
    "Mesajınız şu anda gönderilemedi. Lütfen aşağıdaki WhatsApp veya e-posta seçeneğini kullanın; hemen size dönelim.",
  "Prefer to write directly?": "Doğrudan yazmayı mı tercih edersiniz?",

  /* ---------- booking ---------- */
  "Book Karibu Assalam | Retreats, Rooms, and Campus Visits":
    "Karibu Assalam Rezervasyon | Kamplar, Odalar ve Kampüs Ziyaretleri",
  "Send a structured Karibu Assalam booking request for retreats, accommodation, campus visits, airport pickup, and guest support.":
    "Kamplar, konaklama, kampüs ziyaretleri, havalimanı karşılaması ve misafir desteği için ayrıntılı bir Karibu Assalam rezervasyon talebi gönderin.",
  Booking: "Rezervasyon",
  "Start a structured booking request": "Rezervasyon talebinizi oluşturun",
  "Tell us your dates and who is travelling. The team confirms availability and next steps, usually within one day.":
    "Tarihlerinizi ve kimlerin geleceğini yazın. Ekibimiz uygunluğu ve sonraki adımları genellikle bir gün içinde teyit eder.",
  "Room at Karibu Assalam Eco-Village prepared for guests":
    "Karibu Assalam Eco-Village'da misafirler için hazırlanmış oda",
  "Booking request": "Rezervasyon talebi",
  "Share your dates and group details and the team will confirm availability and the next steps.":
    "Tarihlerinizi ve grup bilgilerinizi paylaşın; ekibimiz uygunluğu ve sonraki adımları teyit etsin.",
  "Request type": "Talep türü",
  "Retreat or camp": "Kamp",
  "Not sure yet": "Henüz emin değilim",
  "Arrival date": "Varış tarihi",
  "Departure date": "Ayrılış tarihi",
  "Please choose an arrival date.": "Lütfen bir varış tarihi seçin.",
  "Departure must be after arrival.": "Ayrılış tarihi varıştan sonra olmalı.",
  "At least one adult is required.": "En az bir yetişkin gerekli.",
  Adults: "Yetişkin",
  Children: "Çocuk",
  "Room preference": "Oda tercihi",
  "Preferred language": "Tercih ettiğiniz dil",
  "Preferred contact": "Tercih ettiğiniz iletişim yolu",
  Country: "Ülke",
  "I may need airport pickup": "Havalimanı karşılaması gerekebilir",
  "Dietary or access needs": "Beslenme veya erişilebilirlik ihtiyaçları",
  "Halal meals, vegetarian meals, mobility needs, allergies...":
    "Helal yemek, vejetaryen menü, erişilebilirlik, alerjiler...",
  "Tell us what you want to book, your flexibility, and any questions.":
    "Neyi rezerve etmek istediğinizi, tarih esnekliğinizi ve sorularınızı yazın.",
  "Send request": "Talebi gönder",
  "Sending your request...": "Talebiniz gönderiliyor...",
  "Use direct contact instead": "Bunun yerine doğrudan iletişime geç",
  "Request received. The team will review it shortly.":
    "Talebiniz alındı. Ekibimiz kısa süre içinde inceleyecek.",
  "Thanks — we have your request. The team replies within one day, usually on WhatsApp.":
    "Teşekkürler, talebinizi aldık. Ekibimiz genellikle bir gün içinde, çoğunlukla WhatsApp üzerinden dönüş yapar.",
  "We could not send your request just now. Please send it by WhatsApp or email below and the team will pick it up right away.":
    "Talebiniz şu anda gönderilemedi. Lütfen aşağıdaki WhatsApp veya e-posta seçeneğiyle iletin; ekibimiz hemen ilgilensin.",
  "What happens next": "Sırada ne var",
  "The team checks availability for your dates.": "Ekibimiz tarihleriniz için uygunluğu kontrol eder.",
  "You get a reply with options, pricing, and what is included.":
    "Size seçenekleri, fiyatları ve dahil olan hizmetleri içeren bir yanıt gönderilir.",
  "A 20% deposit confirms a camp booking. Individual stays can be paid on arrival.":
    "Kamp rezervasyonu %20 kaporayla kesinleşir. Bireysel konaklamalar varışta ödenebilir.",
  "Questions in the meantime?": "Bu arada sorunuz olursa",
  "Your trip so far": "Şu ana kadarki planınız",
  Arrival: "Varış",
  Departure: "Ayrılış",
  "Trip nights": "Gece sayısı",
  Guests: "Misafirler",
  "Not set": "Belirtilmedi",
  "guest(s)": "misafir",

  /* ---------- admin (staff-facing) ---------- */
  Admin: "Yönetim",
  "Karibu Assalam Admin | Booking Automation Dashboard":
    "Karibu Assalam Yönetim | Rezervasyon Paneli",
  "Local Karibu Assalam operations dashboard for booking requests, lead triage, status tracking, and reply drafts.":
    "Rezervasyon talepleri, ön değerlendirme, durum takibi ve yanıt taslakları için Karibu Assalam operasyon paneli.",
  "Booking automation dashboard": "Rezervasyon otomasyon paneli",
  "Track inquiries, see automated triage, prepare replies, and move each guest from request to confirmation.":
    "Talepleri takip edin, otomatik önceliklendirmeyi görün, yanıt hazırlayın ve her misafiri talepten onaya taşıyın.",
  "Staff sign in": "Personel girişi",
  "Staff sign in failed.": "Personel girişi başarısız oldu.",
  "Sign out": "Çıkış yap",
  "Sign out failed.": "Çıkış yapılamadı.",
  "Signed out.": "Çıkış yapıldı.",
  Password: "Parola",
  "Operations inbox": "Operasyon kutusu",
  "Operations notes": "Operasyon notları",
  "Booking requests": "Rezervasyon talepleri",
  "Total requests": "Toplam talep",
  "No booking requests yet": "Henüz rezervasyon talebi yok",
  "Booking details and automation notes will appear here.":
    "Rezervasyon detayları ve otomasyon notları burada görünecek.",
  "Select a request": "Bir talep seçin",
  "Loading booking requests...": "Rezervasyon talepleri yükleniyor...",
  "Could not load booking requests.": "Rezervasyon talepleri yüklenemedi.",
  "Booking request could not be saved.": "Rezervasyon talebi kaydedilemedi.",
  "Booking request could not be removed.": "Rezervasyon talebi silinemedi.",
  "Booking request removed.": "Rezervasyon talebi silindi.",
  "Booking status updated.": "Rezervasyon durumu güncellendi.",
  "Booking status could not be updated.": "Rezervasyon durumu güncellenemedi.",
  "Delete request": "Talebi sil",
  "Draft reply": "Yanıt taslağı",
  "Copy draft": "Taslağı kopyala",
  "Reply draft copied.": "Yanıt taslağı kopyalandı.",
  "Copy failed. You can still select the draft manually.":
    "Kopyalanamadı. Taslağı elle seçebilirsiniz.",
  "Add sample inquiry": "Örnek talep ekle",
  "Sample inquiry added for testing the automation dashboard.":
    "Paneli test etmek için örnek talep eklendi.",
  "Sample inquiry could not be added.": "Örnek talep eklenemedi.",
  "Submit a booking form or add a sample inquiry to test the workflow.":
    "Akışı test etmek için bir rezervasyon formu gönderin veya örnek talep ekleyin.",
  "Suggested next action": "Önerilen sonraki adım",
  "Next action": "Sonraki adım",
  "Room planning": "Oda planlaması",
  "Estimated rooms": "Tahmini oda sayısı",
  "Beds requested": "Talep edilen yatak",
  "Guest details": "Misafir bilgileri",
  "Dietary/access": "Beslenme/erişim",
  "Airport pickup": "Havalimanı karşılama",
  "All statuses": "Tüm durumlar",
  Filter: "Filtre",
  Status: "Durum",
  Priority: "Öncelik",
  "High priority": "Yüksek öncelik",
  New: "Yeni",
  Confirmed: "Onaylandı",
  Created: "Oluşturuldu",
  Saved: "Kaydedildi",
  Refresh: "Yenile",
  "Last sync": "Son eşitleme",
  Timing: "Zamanlama",
  Yes: "Evet",
  No: "Hayır",
  "Unnamed guest": "İsimsiz misafir",
  "Open admin dashboard": "Yönetim panelini aç",
  "Cloud sync is temporarily unavailable. Your request is prepared below. Please send it by WhatsApp or email so the team receives it.":
    "Bulut eşitlemesi şu anda kullanılamıyor. Talebiniz aşağıda hazırlandı. Ekibimize ulaşması için lütfen WhatsApp veya e-posta ile gönderin.",

  /* ---------- contact payload wording ---------- */
  "Booking Request": "Rezervasyon Talebi",
  "Contact Request": "İletişim Talebi",
  "General inquiry": "Genel soru",
  Hello: "Merhaba",
  Intent: "Konu",
  "Selected retreat": "Seçilen kamp",
  Name: "Ad",
  "Message:": "Mesaj:",
  "Please share availability and next steps.":
    "Lütfen uygunluk durumunu ve sonraki adımları paylaşın.",

  /* ---------- testimonials ---------- */
  "My experience at Assalam was genuinely special. From the very first day you feel the sincerity and the warm energy there. Seeing people from different cultures come together with good intentions and a beautiful purpose moved me deeply.":
    "Assalam'daki deneyimim benim için gerçekten çok özel. Daha ilk günden oradaki samimiyeti ve güzel enerjiyi hissediyorsunuz. Farklı kültürlerden insanların iyi niyet ve güzel bir amaçla bir araya gelmesi beni çok etkiledi.",
  "I am so glad I could donate, take part in the volunteer activities and travel at the same time. I am very happy to have joined such a beautiful programme and I recommend it to everyone.":
    "Hem bağışımı yapıp hem gönüllü faaliyetlerinde yer alıp hem de gezdiğim için çok memnunum. Böyle güzel bir etkinliğe katıldığım için çok mutluyum ve herkese öneririm.",
  "Paradise-like nature, clean and safe environment, and wonderful people from all over the world with good intentions led to some of the most incredible experiences of my life.":
    "Cennet gibi bir doğa, temiz ve güvenli bir ortam ve dünyanın dört bir yanından gelen iyi niyetli harika insanlar, hayatımın en inanılmaz deneyimlerinden bazılarını yaşattı.",
  "Touching people's hearts in the middle of the Indian Ocean, supporting employment and education, and spending a meaningful week in Zanzibar was like a dream.":
    "Hint Okyanusu'nun ortasında insanların gönüllerine dokunmak, istihdama ve eğitime destek olmak, aynı zamanda Zanzibar'da anlamlı bir hafta geçirmek bir rüya gibiydi.",
  "I genuinely feel like I have a new home. I know I would come back again and again with complete peace of mind.":
    "Kesinlikle yeni bir yuvam var gibi hissediyorum. Tekrar tekrar gönül rahatlığı ile geleceğimi düşünüyorum.",
  "The first day and the introduction were incredible for me. What I saw and heard made me feel I was somewhere very special.":
    "İlk gün ve tanıtım benim için inanılmaz güzeldi. Gördüklerim ve duyduklarım, çok özel bir ortamda olduğumu hissettirdi.",
  Anonymous: "İsimsiz",
  "Kindness Camp guest": "İyilik Kampı misafiri",
  "Ramadan Camp guest": "Ramazan Kampı misafiri",
  "Nature Retreat guest": "Doğa Kampı misafiri",
  "Volunteer programme guest": "Gönüllülük programı misafiri",
  "School Camp guest": "Okul Kampı misafiri",

  /* ---------- added during the design pass ---------- */
  "A typical day-by-day flow. Exact activities are confirmed with the team when you book.": "Tipik bir gün gün akış. Kesin etkinlikler rezervasyon sırasında ekiple birlikte netleşir.",
  "Backend mode": "Veri kaynağı",
  "Check dates and availability": "Tarih ve uygunluk sor",
  "Close menu": "Menüyü kapat",
  "Every camp runs from Karibu Assalam Eco-Village, the only eco-village in Zanzibar. Meals, accommodation, guided tours and airport transfers are included.": "Tüm kamplar Zanzibar'ın tek eko-köyü olan Karibu Assalam Eco-Village'da düzenlenir. Yemekler, konaklama, rehberli turlar ve havalimanı transferi dahildir.",
  "Local fallback": "Yerel yedek",
  "Send it straight to the team": "Doğrudan ekibe gönderin",
  "Signed in. Booking requests loaded from Supabase.": "Giriş yapıldı. Rezervasyon talepleri yüklendi.",
  "Supabase": "Bulut",
  "Use a Supabase Auth staff account that has a matching row in the staff profiles table.": "Personel profilleri tablosunda kaydı bulunan bir personel hesabıyla giriş yapın.",
  "You can also forward this request yourself:": "Bu talebi kendiniz de iletebilirsiniz:",
  "Follow us on Instagram": "Instagram'da takip edin",

  /* ---------- booking form options ---------- */
  "Accommodation only": "Sadece konaklama",
  "Campus visit": "Kampüs ziyareti",
  "Group or school program": "Grup veya okul programı",
  "General travel question": "Genel seyahat sorusu",
  "Phone call": "Telefon görüşmesi",
  "English": "İngilizce",
  "French": "Fransızca",
  "German": "Almanca",
  "Italian": "İtalyanca",
  "Turkish": "Türkçe",
  "Swahili": "Svahili",
  "Arabic": "Arapça",
  "Shared eco-village room": "Paylaşımlı eko-köy odası",
  "Family or private room request": "Aile veya özel oda talebi",
  "Group room block": "Grup için oda bloğu",
  "Contacted": "İletişime geçildi",
  "Quoted": "Teklif verildi",
  "Deposit pending": "Kapora bekleniyor",
  "Cancelled": "İptal edildi",
  "Room estimates assume the standard 5-bed room.": "Oda tahminleri standart 5 yataklı odaya göre yapılır.",

  /* ---------- showcase rows: retreats and rooms ---------- */
  "25 m² rooms": "25 m² odalar",
  "25 m², up to five beds, a bathroom and a balcony.": "25 m², beş yatağa kadar, banyo ve balkon.",
  "A campus you can walk across in five minutes.": "Beş dakikada baştan başa yürüyebileceğiniz bir kampüs.",
  "A simple room, a short walk from the Indian Ocean": "Sade bir oda, Hint Okyanusu’na birkaç adım",
  "Air conditioning": "Klima",
  "Bed configurations that suit families, groups and school camps.": "Ailelere, gruplara ve okul kamplarına uygun yatak düzenleri.",
  "Each camp keeps the same rhythm — guided days, shared meals, time with the community — and changes what sits at its centre.": "Her kamp aynı ritmi korur — rehberli günler, ortak sofralar, toplulukla geçen zaman — ve merkezine koyduğu şeyi değiştirir.",
  "Every room is air-conditioned, with mosquito nets on the beds.": "Tüm odalar klimalıdır ve yataklarda cibinlik bulunur.",
  "Ocean on one side, garden on the other.": "Bir yanda okyanus, diğer yanda bahçe.",
  "Ocean or garden": "Okyanus ya da bahçe",
  "Room and bathroom, with space for luggage and group kit.": "Oda ve banyo; bagaj ve grup malzemesi için yer.",
  "Rooms look out over the ocean or into the garden, each with a balcony.": "Odalar okyanusa ya da bahçeye bakar; her birinde balkon vardır.",
  "See the campus": "Kampüsü görün",
  "Six ways to spend a week in Zanzibar": "Zanzibar’da bir haftayı geçirmenin altı yolu",
  "Solar powered": "Güneş enerjisi",
  "Steps from the sea": "Denize birkaç adım",
  "The eco-village around it": "Çevresindeki eko-köy",
  "The lodge runs on renewable energy systems, including solar.": "Tesis, güneş enerjisi dahil yenilenebilir enerji sistemleriyle çalışır.",
  "The lodge sits on the oceanfront, ten minutes’ walk from the sandy beach.": "Tesis okyanus kıyısındadır; kum plaja yürüyerek on dakikadır.",
  "The room": "Oda",
  "The view": "Manzara",
  "There is one kind of room at Karibu Assalam Eco-Village, and it is built for the way people actually stay here: in groups, for a week at a time, spending most of the day outside.": "Karibu Assalam Eco-Village’da tek tip oda vardır ve burada gerçekten nasıl kalındığına göre tasarlanmıştır: gruplar hâlinde, birer haftalık, günün büyük bölümü dışarıda.",
  "Up to 5 beds": "5 yatağa kadar",
  "What is in every room": "Her odada ne var",
  "Where you sleep": "Nerede kalıyorsunuz",
  "Zanzibar beach near Karibu Assalam Eco-Village": "Karibu Assalam Eco-Village yakınındaki Zanzibar plajı",

  /* ---------- hero slider and dock ---------- */
  "Karibu Assalam Eco-Village, Zanzibar": "Karibu Assalam Eco-Village, Zanzibar",
  "Choose a retreat": "Bir kamp seçin",
  "More info": "Daha fazla bilgi",
  "Need help with your booking?": "Rezervasyonunuz için yardım ister misiniz?",
  "Next slide": "Sonraki görsel",
  "Previous slide": "Önceki görsel",
  "View more": "Daha fazlası",

  /* ---------- experiences, campers, volunteering ---------- */
  "A day on the water — snorkelling, sandbanks and marine life, by traditional dhow.": "Geleneksel bir dhow ile suda geçen bir gün — şnorkel, kum adacıkları ve deniz yaşamı.",
  "A free guided walk through the eco-village, every day.": "Her gün, eko-köyde ücretsiz rehberli bir yürüyüş.",
  "A member of the team walks you through the campus: the school, the permaculture garden, the solar and water systems, the kitchen and the workshop spaces. It is the quickest way to understand how the village actually works.": "Ekibimizden biri sizi kampüste gezdirir: okul, permakültür bahçesi, güneş enerjisi ve su sistemleri, mutfak ve atölye alanları. Köyün nasıl işlediğini anlamanın en hızlı yolu budur.",
  "Across the island": "Ada genelinde",
  "An annual festival of Sufi music, poetry and gathering, hosted with Assalam. It has its own site with the programme and dates.": "Assalam ile birlikte düzenlenen yıllık bir Sufi müziği, şiir ve buluşma festivali. Program ve tarihler için kendi sitesi var.",
  "Apply to volunteer": "Gönüllü olmak için başvurun",
  "Arriving in your own camper": "Kendi karavanınızla gelmek",
  "Ask about safari": "Safari hakkında sorun",
  "Ask the team first": "Önce ekibe danışın",
  "Bathrooms, the kitchen and the communal areas are shared with everyone staying on site.": "Banyolar, mutfak ve ortak alanlar tesiste kalan herkesle paylaşılır.",
  "Because camper space depends on the week, please confirm with the team before you set off rather than arriving unannounced.": "Karavan alanı haftaya göre değiştiği için, habersiz gelmek yerine yola çıkmadan önce lütfen ekiple teyitleşin.",
  "Before you come": "Gelmeden önce",
  "Beyond the island": "Adanın ötesinde",
  "Book a tour": "Tur rezervasyonu",
  "Book a workshop": "Atölye rezervasyonu",
  "Book an experience": "Deneyim rezervasyonu",
  "Camper parking inside the eco-village": "Eko-köy içinde karavan alanı",
  "Campers are welcome at the daily campus tour, the workshops and the shared meals.": "Karavanla gelenler günlük kampüs turuna, atölyelere ve ortak sofralara katılabilir.",
  "Campers at Karibu Assalam Eco-Village | Karibu Assalam": "Karibu Assalam Eco-Village'da Karavanlar | Karibu Assalam",
  "Camps and retreats": "Kamplar ve retretler",
  "City Tour": "Şehir Turu",
  "Cook Zanzibari dishes with the kitchen team — spices from the garden, recipes from the neighbourhood, and the meal is lunch.": "Mutfak ekibiyle Zanzibar yemekleri pişirin — bahçeden baharatlar, mahalleden tarifler; pişen yemek de öğle yemeği olur.",
  "Cooking class": "Yemek atölyesi",
  "Daily campus tour": "Günlük kampüs turu",
  "Daily campus tours, hands-on workshops, Zanzibar tours, safari, volunteering and special events at Karibu Assalam.": "Karibu Assalam'da günlük kampüs turları, uygulamalı atölyeler, Zanzibar turları, safari, gönüllülük ve özel etkinlikler.",
  "Dates are announced as they are confirmed. Follow along or ask the team to let you know when something is opening.": "Tarihler kesinleştikçe duyurulur. Bizi takip edin ya da yeni bir program açıldığında haber vermemizi isteyin.",
  "Details are confirmed with the team when you enquire — space is limited and depends on what else is running that week.": "Detaylar başvurunuzda ekiple netleşir — alan sınırlıdır ve o hafta yürüyen programlara bağlıdır.",
  "Drum workshop": "Ritim atölyesi",
  "Eco print workshop": "Eko baskı atölyesi",
  "Every day at Karibu Assalam Eco-Village has something on it — a walk around the campus, a workshop, a tour across the island, or an evening of music.": "Karibu Assalam Eco-Village'da her günün bir programı vardır — kampüste bir yürüyüş, bir atölye, adada bir tur ya da müzikli bir akşam.",
  "Experiences in Zanzibar | Karibu Assalam": "Zanzibar'da Deneyimler | Karibu Assalam",
  "Festivals, music and the programmes that bring people to the village from across the island and beyond.": "Adanın her yerinden ve ötesinden insanları köye getiren festivaller, müzik ve programlar.",
  "Give your time": "Zamanınızı ayırın",
  "Guided days out from the eco-village, with transport and a guide from the team.": "Eko-köyden çıkan rehberli günübirlik turlar; ulaşım ve rehber ekipten.",
  "Half-day sessions run on the campus with the people who do this work every day. Book them on their own or as part of a camp.": "Kampüste, bu işi her gün yapan kişilerle yürütülen yarım günlük oturumlar. Tek başına ya da bir kamp kapsamında alınabilir.",
  "Hands on": "Uygulamalı",
  "Join an existing programme for a few weeks — teaching support, the permaculture garden, workshops or events. Best suited to travellers who want to contribute alongside a normal stay.": "Birkaç haftalığına yürüyen bir programa katılın — eğitim desteği, permakültür bahçesi, atölyeler ya da etkinlikler. Normal bir konaklamanın yanında katkı sunmak isteyenler için uygundur.",
  "Join the programme": "Programa katılın",
  "Learn Swahili coastal rhythms with local musicians. No experience needed — everyone leaves able to hold a pattern.": "Yerel müzisyenlerle Svahili sahil ritimlerini öğrenin. Deneyim gerekmez — herkes bir ritmi tutturarak ayrılır.",
  "Long-term volunteering": "Uzun süreli gönüllülük",
  "Longer placements take on real responsibility inside a project — a class, a garden, a workshop programme. These are arranged case by case, and start with a conversation about your skills and dates.": "Uzun süreli katılımlar bir proje içinde gerçek sorumluluk üstlenir — bir sınıf, bir bahçe, bir atölye programı. Her biri ayrı ayrı planlanır ve becerileriniz ile tarihleriniz üzerine bir görüşmeyle başlar.",
  "Mainland Tanzania, arranged from Zanzibar.": "Zanzibar'dan düzenlenen Tanzanya anakara turları.",
  "Make your own soap with the women's cooperative, using coconut oil and local botanicals. You take your bars home with you.": "Kadın kooperatifiyle birlikte, hindistan cevizi yağı ve yerel bitkilerle kendi sabununuzu yapın. Sabunlarınızı yanınızda götürürsünüz.",
  "Multi-day safari trips to the mainland parks can be arranged around your stay. Because routes, seasons and prices change, the team plans each one with you directly rather than selling a fixed package.": "Anakaradaki parklara birkaç günlük safari turları konaklamanıza göre düzenlenebilir. Rotalar, mevsimler ve fiyatlar değiştiği için ekip hazır paket satmak yerine her turu sizinle birlikte planlar.",
  "Park on the campus": "Kampüste konaklayın",
  "Photograph coming soon": "Fotoğraf yakında",
  "Print fabric with leaves, flowers and bark gathered on the campus, using natural dyes and no synthetic chemicals.": "Kampüsten toplanan yaprak, çiçek ve ağaç kabuğuyla, doğal boyalar kullanarak ve sentetik kimyasal olmadan kumaş baskısı yapın.",
  "Read our reviews on Tripadvisor": "Tripadvisor'daki yorumlarımızı okuyun",
  "Ready to plan your days?": "Günlerinizi planlamaya hazır mısınız?",
  "Room for your own vehicle": "Kendi aracınız için yer",
  "Safari": "Safari",
  "Safari in mainland Tanzania": "Tanzanya anakarasında safari",
  "Sawa Ensemble": "Sawa Ensemble",
  "Short-term volunteering": "Kısa süreli gönüllülük",
  "Soap making": "Sabun atölyesi",
  "Space for campers inside the Karibu Assalam eco-village, with shared facilities and access to the daily programme.": "Karibu Assalam eko-köyünde karavanlar için alan; ortak olanaklar ve günlük programa katılım imkânı.",
  "Space to park inside the eco-village, within the same grounds as the lodge and the school.": "Eko-köy içinde, tesis ve okulla aynı alanda park yeri.",
  "Special events": "Özel etkinlikler",
  "Spice Tour": "Baharat Turu",
  "Start here": "Buradan başlayın",
  "Stay updated": "Gelişmelerden haberdar olun",
  "Stone Town on foot: the old fort, the markets, the carved doors and the seafront at sunset.": "Yürüyerek Stone Town: eski kale, pazarlar, oymalı kapılar ve gün batımında sahil.",
  "Tell the team your dates and what you would like to do. They will put the week together with you and confirm what is available.": "Ekibe tarihlerinizi ve yapmak istediklerinizi yazın. Haftanızı sizinle birlikte planlayıp uygunluğu teyit etsinler.",
  "The east side of the island, its beaches and its villages, including the Rock Restaurant when the tide allows.": "Adanın doğu yakası, plajları ve köyleri; gelgit uygun olduğunda Rock Restaurant dahil.",
  "The music ensemble that grew out of the campus, performing coastal and devotional repertoire. Follow them for performance dates.": "Kampüsten doğan, sahil ve tasavvuf repertuvarı seslendiren müzik topluluğu. Konser tarihleri için takip edin.",
  "The scheduled programmes that run through the year — kindness camps, Ramadan camps, school camps and nature retreats.": "Yıl boyunca yürüyen programlar — iyilik kampları, Ramazan kampları, okul kampları ve doğa retretleri.",
  "There is room inside the eco-village for travellers arriving with their own vehicle.": "Eko-köy içinde, kendi aracıyla gelen misafirler için yer vardır.",
  "Things to do from the eco-village": "Eko-köyden yapılabilecekler",
  "Through the year": "Yıl boyunca",
  "Two ways to join the work, depending on how long you can stay. Both start with a conversation about your skills and your dates.": "Kalış sürenize göre çalışmaya katılmanın iki yolu. İkisi de becerileriniz ve tarihleriniz üzerine bir görüşmeyle başlar.",
  "Use the village facilities": "Köyün olanaklarını kullanın",
  "Volunteering": "Gönüllülük",
  "Walk a working spice farm and meet the plants behind the island's name — clove, cardamom, vanilla, nutmeg, cinnamon.": "Faal bir baharat çiftliğinde yürüyün ve adaya adını veren bitkilerle tanışın — karanfil, kakule, vanilya, küçük hindistan cevizi, tarçın.",
  "Workshops": "Atölyeler",
  "Zanzibar Sufi Festival": "Zanzibar Sufi Festivali",
  "Zanzibar tours": "Zanzibar turları",
  "Space inside the eco-village for travellers arriving with their own vehicle, with shared facilities.": "Eko-köy içinde, kendi aracıyla gelen misafirler için ortak olanaklara sahip alan.",

  /* ---------- experience facts and link labels ---------- */
  "3 months and over": "3 ay ve üzeri",
  "About 45 minutes": "Yaklaşık 45 dakika",
  "Arranged on request": "Talep üzerine düzenlenir",
  "Ask about dates": "Tarihleri sorun",
  "Follow on Instagram": "Instagram’da takip edin",
  "Free for guests": "Misafirler için ücretsiz",
  "Multi-day": "Birkaç günlük",
  "Planned with the team": "Ekiple birlikte planlanır",
  "Runs daily": "Her gün",
  "See the retreats": "Kampları görün",
  "Under 3 months": "3 aydan kısa",
  "Visit the festival site": "Festival sitesini ziyaret edin",

  "Close": "Kapat",
  "View photo": "Fotoğrafı görüntüle",
  "Assalam Community Foundation community programme in Zanzibar": "Assalam Community Foundation community programme in Zanzibar",
  "Beachfront swing and gathering area at Karibu Assalam Eco-Village": "Beachfront swing and gathering area at Karibu Assalam Eco-Village",
  "Book your retreat": "Book your retreat",
  "Community education and volunteering at Assalam Community Foundation": "Community education and volunteering at Assalam Community Foundation",
  "Find Your Stay": "Find Your Stay",
  "Guests taking part in a Karibu Assalam retreat": "Guests taking part in a Karibu Assalam retreat",
  "Historic Stone Town in Zanzibar": "Historic Stone Town in Zanzibar",
  "Image placeholder for the halal spa and pool": "Image placeholder for the halal spa and pool",
  "Karibu Assalam Eco-Village accommodation in Kizimkazi": "Karibu Assalam Eco-Village accommodation in Kizimkazi",
  "More details": "More details",
  "Nature and sustainable living at Karibu Assalam": "Nature and sustainable living at Karibu Assalam",
  "Prepared guest room at Karibu Assalam Eco-Village": "Prepared guest room at Karibu Assalam Eco-Village",
  "Visitors connecting during a Karibu Assalam community retreat": "Visitors connecting during a Karibu Assalam community retreat",
};

